...
class KamiMusicLyric {
  ...
  getIndex(time) {
    for (let i = 0, n = this.lyrics.length; i < n; i++) {
      ...
      if (time >= this.lyrics[i].start && time <= this.lyrics[i].end) {
        ...
        time > this.lyrics[i].end &&
        time < (this.lyrics[i + 1]?.start ?? Infinity)
      ) {
        return i + 0.5;
      }
    }
  ...
  static async fetchLyric(id) {
    const res = await fetch(
      "https://www.jpmarumaru.com/tw/api/json_JPSongTrack.asp",
      {
        headers: {
    ...
    for (const i in LyricsYomi) {
      const matches = LyricsYomi[i].matchAll(
  ...
  static async searchLyrics(kw) {
    try {
      ...
      const term = kw.replace(
        /(cover(?:ed)?(?:\sby)?.+)|(official(?:\s?music)?(?:\s?video)?)|(music(?:\s?video)?)|([「『【｛（［({<].*[」』】｝）］)}>])|(-)/gi,
        ""
      );
      ...
      if (raw.results?.length) {
        ...
        for (let i = 0, n = raw.results.length; i < n; i++) {
          ...
          if (e.url.startsWith("https://www.jpmarumaru.com/tw/JPSongPlay-")) {
            result.push({
              name: e.titleNoFormatting.replace(
                "-歌詞-唱歌學日語-日語教室-MARUMARU",
                ""
              ),
              url: e.url,
              titlePredict: e.titleNoFormatting
                .replace("-歌詞-唱歌學日語-日語教室-MARUMARU", "")
                .split("-")
                .slice(0, -1)
                .join(""),
              artistPredict: e.titleNoFormatting
                .replace("-歌詞-唱歌學日語-日語教室-MARUMARU", "")
                .split("-")
                .slice(1)
                .join(""),
              id: e.url.replace(
                /https:\/\/www\.jpmarumaru\.com\/tw\/JPSongPlay-(.+)\.html/,
                "$1"
              ),
            });
          }
...
function searchPredefinedDictionary(kw) {
  const dictionary = JSON.parse(
    readFileSync(join(__dirname, "..", "Database", "lyrics.json"), {
      encoding: "utf-8",
    })
  );
  const results = [];
  ...
  for (let i = 0, n = dictionary.length; i < n; i++) {
    ...
    if (
      !(kw.includes(e.titlePredict) && kw.includes(e.artistPredict)) &&
      kw.includes(e.titlePredict)
    ) {
      results.push(e);
    }
  }
