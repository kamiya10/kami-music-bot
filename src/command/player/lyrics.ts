  ...
  async execute(interaction) {
    try {
      ...
      let embed = new EmbedBuilder()
        ...
        .setAuthor({
          name: `歌詞 | ${interaction.guild.name}`,
        ...
        .setFooter({
          text: "歌詞提供：Genius",
          iconURL: "https://upload.cc/i1/2022/09/11/KmhuEB.png",
        })
        .setTimestamp();
      ...
      if (resource) {
        try {
          const song = (
            await genius.songs.search(GuildMusicPlayer.current.title)
          )[0];

          const lyrics = await song.lyrics();

          embed = embed.setDescription(
            lyrics.split("/n").slice(2).join("\n").trim()
          );
        } catch (e) {
          embed = embed
            .setURL(GuildMusicPlayer.current.url)
