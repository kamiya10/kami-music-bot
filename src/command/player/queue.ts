  ...
  async execute(interaction) {
    try {
      ...
      const listMapper = (v, i, a) =>
        (i == GuildMusicPlayer.currentIndex &&
        GuildMusicPlayer.player.state.status != AudioPlayerStatus.Idle
          ? GuildMusicPlayer.paused
            ? "⏸️ **"
            : GuildMusicPlayer.repeat == RepeatMode.RepeatCurrent
              ? "🔂 **"
              : GuildMusicPlayer.repeat == RepeatMode.Random ||
                  GuildMusicPlayer.repeat == RepeatMode.RandomNoRepeat
                ? "🔀 **"
                : "▶️ **"
          : v.error
            ? "❌ "
        ...
        (!v.playable ? "~~" : "") +
        (i == GuildMusicPlayer.currentIndex &&
        GuildMusicPlayer.player.state.status != "idle"
          ? "**"
          : "") +
      ...
      let paginator = new ActionRowBuilder().setComponents(
        ...[
          new ButtonBuilder()
            .setCustomId("first")
            .setEmoji("⏮")
      ...
      collector.on("collect", async (i) => {
        if (i.member.id == interaction.member.id) {
          ...
          paginator = paginator.setComponents(
            ...[
              new ButtonBuilder()
                .setCustomId("first")
                .setEmoji("⏮")
          ...
          const embed = new EmbedBuilder()
            ...
            .setDescription(
              `${i.client.EmbedIcon.Error} 你沒有權限和這個訊息互動`
            );
      ...
      collector.on("end", async (c, reason) => {
        if (reason == "idle") {
          await sent.edit({
            embeds: [queue.setFooter({ text: "互動已逾時" })],
          });
