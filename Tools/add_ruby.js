...
function parseRuby(filename) {
  const data = readFileSync(
    join(__dirname, filename.replace(".lyric", "_ruby.txt")),
    { encoding: "utf-8" }
  )
    .split(EOL)
    .slice(1)
    ...
    for (const match of matches) {
      ...
      if (!match[2])
        throw new SyntaxError(
          `Missing ruby text in match ${match[0]}, line ${+i + 2}`
        );
      ...
      else space = "　".repeat(Math.abs(match[1].length - match[2].length) / 2);
